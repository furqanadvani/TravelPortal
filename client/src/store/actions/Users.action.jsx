...
export const updateUserPersonalDetails = (payload, CB) => async (dispatch) => {
    const dispatchType = USERS.UPDATE_USER_PERSONAL_DETAILS
    dispatch({ type: dispatchType, loading: true });
    try {
        const { data } = await post(`/onboarding/user-personal-details`, payload)
        if (!data.error) {
            dispatch({ type: dispatchType, loading: false });
            handleSuccess(data.message)
            CB && CB()
...
export const saveEmployementDetails = (payload, CB) => async (dispatch) => {
    const dispatchType = USERS.SAVE_EMPLOYEMENT_DETAILS
    dispatch({ type: dispatchType, loading: true });
    try {
        const { data } = await post(`/onboarding/employment-details`, payload)
        if (!data.error) {
            dispatch({ type: dispatchType, loading: false });
            handleSuccess(data.message)
            CB && CB()
...
export const updateUserAccessReqForm = (payload, CB) => async (dispatch) => {
    const dispatchType = USERS.UPDATE_USER_ACCESS_RIGHTS
    dispatch({ type: dispatchType, loading: true });
    try {
        const { data } = await post(`/onboarding/user-access`, payload)
        if (!data.error) {
            dispatch({ type: dispatchType, loading: false });
            handleSuccess(data.message)
            CB && CB()
