  ...
  try {
    ...
    if (!data.error && data?.token) {
      if (data?.isFirstLogin) {
        ...
        CB && CB(data?.changePasswordToken);
      } else {
    ...
    if (error?.name === "CanceledError" || error?.canceled) {
      return;
    }
    dispatch({ type: dispatchType, loading: false, isLoggedIn: false, });
...
export const getProfile = (isRetry = false) => async (dispatch) => {
  ...
  try {
    ...
    if (!data.error) {
      ...
      if (!isRetry) {
        setTimeout(() => dispatch(getProfile(true)), 1000);
      } else {
        dispatch({ type: dispatchType, data: {}, loading: false, isLoggedIn: false });
        dispatch(logout());
      }
    }
  } catch (error) {
